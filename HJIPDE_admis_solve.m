function [data, tau, extraOuts] = ...
    HJIPDE_admis_solve(data0, tau, schemeData, compMethod, extraArgs)
% [data, tau, extraOuts] = ...
%   HJIPDE_admis_solve(data0, tau, schemeData, minWith, extraargs)
...
if (isfield(extraArgs, 'visualize') && isstruct(extraArgs.visualize))...
    ...
    if isfield(extraArgs.visualize, 'xTitle')
        xlabel(extraArgs.visualize.xTitle)
    end
    
    if isfield(extraArgs.visualize, 'yTitle')
        ylabel(extraArgs.visualize.yTitle)
    end
    
    if isfield(extraArgs.visualize, 'zTitle')
        zlabel(extraArgs.visualize.zTitle)
    end
...
extraOuts.u_min = zeros([data0size(1:gDim) length(tau)]);
extraOuts.u_max = zeros([data0size(1:gDim) length(tau)]);
extraOuts.grad1 = zeros([data0size(1:gDim) length(tau)]);
extraOuts.grad2 = zeros([data0size(1:gDim) length(tau)]);
...
%         end
    ...
    [u_ad_min, u_ad_max, grad1, grad2] = dynSys.admis(y0, y, schemeData, tNow, tau(i)-tau(i-1));
    ...
    extraOuts.u_min(clns{:}, i) = u_ad_min;
    extraOuts.u_max(clns{:}, i) = u_ad_max;
    extraOuts.grad1(clns{:}, i) = grad1;
    extraOuts.grad2(clns{:}, i) = grad2;
...
% extraOuts.u_min(clns{:}, 1) = [];
% extraOuts.u_max(clns{:}, 1) = [];
