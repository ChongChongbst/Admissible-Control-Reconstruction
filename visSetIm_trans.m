function h = visSetIm_trans(g, data, color, level, extraArgs)
...
if isfield(extraArgs, 'FaceAlpha')
  FaceAlpha = extraArgs.FaceAlpha;
end
...
switch g.dim
  ...
  case 3
    h = visSetIm3D(g, data, color, level, applyLight, FaceAlpha);
    
  case 4
    h = visSetIm4D(g, data, color, level, sliceDim, applyLight, FaceAlpha);
end
end

%% 3D Visualization
function h = visSetIm3D(g, data, color, level, applyLight, FaceAlpha)
...
xlim([-2 2]);
ylim([-2 2]);
xlabel('-2<p_x<2','fontsize',20);
ylabel('-2<p_y<2','fontsize',20);
zlabel('-\pi<\theta<\pi','fontsize',20);
...
%% 4D Visualization
function h = visSetIm4D(g, data, color, level, sliceDim, applyLight, FaceAlpha)
...
for i = 1:N
  ...
  % Visualize 3D slices
  h{i} = visSetIm3D(g3D, data3D, color, level, applyLight, FaceAlpha);
